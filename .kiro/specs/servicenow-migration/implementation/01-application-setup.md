...
2. **Create Properties**
   ...
   |------|-------|-------------|
   | x_ec_audit.cris.endpoint | https://cris.example.ec.europa.eu/api | CRIS REST API endpoint |
   | x_ec_audit.summa.endpoint | https://summa.example.ec.europa.eu/api | SUMMA REST API endpoint |
   | x_ec_audit.hrs.endpoint | https://hrs.example.ec.europa.eu/api | HRS REST API endpoint |
   | x_ec_audit.default.year | (leave empty) | Default audit year (uses current if empty) |
...
✅ Created scoped application `x_ec_audit`
...
- `x_ec_audit_t_audits` (from Oracle `T_AUDITS`)
- `x_ec_audit_t_audit_results` (from Oracle `T_AUDIT_RESULTS`)
- `x_ec_audit_t_cref_audit_types` (from Oracle `T_CREF_AUDIT_TYPES`)
- etc.
...
Proceed to Task 1.2: Create the main audit table `T_AUDITS` (x_ec_audit_t_audits)
